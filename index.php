...
$GLOBALS['xoopsOption']['template_main'] = 'portfolio_index.tpl';
...
while ($row = $db->fetchArray($result)) {
    $tpl->append('categos', array(
        'id'     => $row['id_cat'],
        'nombre' => $row['nombre'],
        'desc'   => $myts->displayTarea($row['desc'], 1, 1, 1, 1, 1)
    ));
}
...
while ($row = $db->fetchArray($result)) {
    $tpl->append('destacados', array(
        'id'     => $row['id_w'],
        'titulo' => $row['titulo'],
        'desc'   => $myts->displayTarea($row['short'], 1, 1, 1, 1, 1),
        'img'    => $row['imagen']
    ));
}
...
while ($row = $db->fetchArray($result)) {
    $tpl->append('recientes', array(
        'id'     => $row['id_w'],
        'titulo' => $row['titulo'],
        'desc'   => $myts->displayTarea($row['short'], 1, 1, 1, 1),
        'img'    => $row['imagen']
    ));
}
