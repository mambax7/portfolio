        ...
        foreach ($this->_options as $k => $v) {
            ...
            switch ($k) {
                case 'day':
                    ...
                    for ($i = 1; $i <= 31; ++$i) {
                        $ret .= "<option value='$i'" . (($i == $day) ? ' selected' : '') . ">$i</option>";
                    }
                    $ret .= '</select>';
                    break;
                case 'month':
                    ...
                    for ($i = 1; $i <= 12; ++$i) {
                        $ret .= "<option value='$i'" . ((date($v['format'], mktime(0, 0, 0, $i, 1, 0)) == $month) ? ' selected' : '') . '>' . date($v['format'], mktime(0, 0, 0, $i, 1, 0)) . '</option>';
                    }
                    $ret .= '</select> ';
                    break;
                case 'year':
                    ...
                    for ($i = $v['start']; $i <= $v['end']; ++$i) {
                        $ret .= "<option value='$i'" . ((date($v['format'], mktime(0, 0, 0, 1, 1, $i)) == $year) ? ' selected' : '') . '>' . date($v['format'], mktime(0, 0, 0, 1, 1, $i)) . '</option>';
                    }
                    $ret .= '</select> ';
                    break;
                case 'hour':
                    ...
                    for ($i = 0; $i <= 23; ++$i) {
                        $ret .= "<option value='$i'" . ((date($v['format'], mktime($i, 0, 0, 1, 1, 0)) == $hour) ? ' selected' : '') . '>' . date($v['format'], mktime($i, 0, 0, 1, 1, 0)) . '</option>';
                    }
                    $ret .= '</select> ';
                    break;
                case 'minute':
                    ...
                    for ($i = 0; $i <= 59; ++$i) {
                        $ret .= "<option value='$i'" . ((date($v['format'], mktime(0, $i, 0, 1, 1, 0)) == $min) ? ' selected' : '') . '>' . date($v['format'], mktime(0, $i, 0, 1, 1, 0)) . '</option>';
                    }
                    $ret .= '</select> ';
                    break;
                case 'second':
                    ...
                    for ($i = 0; $i <= 59; ++$i) {
                        $ret .= "<option value='$i'" . ((date($v['format'], mktime(0, 0, $i, 1, 1, 0)) == $second) ? ' selected' : '') . '>' . date($v['format'], mktime(0, 0, $i, 1, 1, 0)) . '</option>';
                    }
                    $ret .= '</select> ';
                    break;
            }
