...
$adminObject->addItemButton(_MI_PORTFOLIO_AM2, 'main.php?op=new', 'add', '');
...
function portfolioShow()
{
    ...
    require_once __DIR__ . '/../class/catego.class.php';
    $table = new MFTable(true);
    $table->setCellStyle('padding: 0px; padding-left: 10px; padding-right: 10px; vertical-align: middle; border-bottom: 1px solid #0066CC; border-right: 1px solid #0066CC; background: url(../assets/images/bgth.jpg) repeat-x; height: 20px; color: #FFFFFF;');
    $table->openTbl('100%', '', 1);
    $table->openRow('left');
    $table->addCell(_MA_PORTFOLIO_WORKS, 1, 4);
    $table->closeRow();
    $table->setRowClass('head');
    $table->setCellStyle('padding: 0px; padding-left: 3px; padding-right: 3px; border-bottom: 1px solid #DBE691; border-right: 1px solid #DBE691; background: url(../assets/images/bghead.jpg) repeat-x; height: 20px; color: #000000;');
...
function portfolioNew()
{
    ...
    xoops_cp_header();
    //portfolio_make_adminnav();
    $adminObject->displayNavigation('main.php?op=new');

    require_once __DIR__ . '/../common/form.class.php';
...
function portfolioSave()
{
    ...
    if ($catego <= 0) {
        redirect_header('?op=new', 1, _MA_PORTFOLIO_ERRCATEGO);
        die();
    }
    //  if ($cliente=='') { redirect_header('?op=new', 1, _MA_PORTFOLIO_ERRCLIENTE); die(); }
    if ($desc == '') {
        redirect_header('?op=new', 1, _MA_PORTFOLIO_ERRDESC);
        die();
    }
    ...
    if (is_uploaded_file($_FILES['imagen']['tmp_name'])) {
        $upload = new XoopsMediaUploader($mc['storedir'], array(
            'image/gif',
            'image/jpeg',
            'image/pjpeg',
            'image/jpg',
...
function portfolioEdit()
{
    ...
    require_once __DIR__ . '/../class/work.class.php';
    xoops_cp_header();
    //portfolio_make_adminnav();
    ...
    portfolio_get_categos($result);
    foreach ($result as $k => $v) {
        $select .= "<option value='$v[id_cat]'" . (($v['id_cat'] == $work->getVar('catego')) ? ' selected' : '') . '>' . str_repeat('-', $v['saltos']) . " $v[nombre]</option>";
    }
    $select .= '</select>';
    $form->addElement(new RMLabel(_MA_PORTFOLIO_CATEGO, $select));
    ...
    $form->addElement(new RMTextArea(_MA_PORTFOLIO_SHORT, 'short', 4, 45, $myts->htmlSpecialChars($work->getVar('short'), 0, 0)));
    ...
    //$form->addElement(new RMLabel(_MA_PORTFOLIO_DESC, portfolio_select_editor('desc',$mc['editor'],$myts->htmlSpecialChars($work->getVar('desc')),'100%','250px')));
    $form->addElement(new RMLabel(_MA_PORTFOLIO_DESC, $formmnote->render()));
...
function portfolioSaveEdit()
{
    ...
    if ($catego <= 0) {
        redirect_header("?op=edit&amp;id=$id", 1, _MA_PORTFOLIO_ERRCATEGO);
        die();
    }
    //  if ($cliente=='') { redirect_header("?op=edit&amp;id=$id", 1, _MA_PORTFOLIO_ERRCLIENTE); die(); }
    if ($desc == '') {
        redirect_header("?op=edit&amp;id=$id", 1, _MA_PORTFOLIO_ERRDESC);
        die();
    }
...
function portfolioAddImgs()
{
    ...
    include __DIR__ . '/../class/work.class.php';
    $work = new MFWork($id);
...
function portfolioDelImg()
{
    ...
    if ($img <= 0) {
        header("location: main.php?op=imgs&amp;id=$id");
        die();
    }
