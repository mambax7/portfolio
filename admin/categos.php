...
$adminObject->addItemButton(_MI_PORTFOLIO_AM4, 'categos.php?op=new', 'add', '');
...
function portfolioShow()
{
    ...
    $table->setCellStyle('padding: 0px; padding-left: 10px; border-bottom: 1px solid #0066CC; border-right: 1px solid #0066CC; background: url(../assets/images/bgth.jpg) repeat-x; height: 20px; color: #FFFFFF;');
    $table->openTbl('100%', '', 1);
    $table->openRow('left');
    ...
    foreach ($result as $k => $v) {
        $table->openRow();
        $table->addCell((($v['saltos'] <= 0) ? "<img src='../assets/images/plus.gif' border='0' align='absmiddle'>" : str_repeat('&nbsp;', $v['saltos']) . "<img src='../assets/images/root.gif' border='0' align='absmiddle'>") . " <strong>$v[nombre]</strong>", 0, '', 'left');
...
function portfolioNew()
{
    ...
    xoops_cp_header();
    //portfolio_make_adminnav();
    $adminObject->displayNavigation('categos.php?op=new');
...
function portfolioEdit()
{
    ...
    portfolio_get_categos($result);
    foreach ($result as $k => $v) {
        $select .= "<option value='$v[id_cat]'" . (($v['id_cat'] == $catego->getVar('parent')) ? ' selected' : '') . ">$v[nombre]</option>";
    }
    $select .= '</select>';
    $form->addElement(new RMLabel(_MA_PORTFOLIO_PARENT, $select));
    ...
    //$form->addElement(new RMLabel(_MA_PORTFOLIO_DESC, portfolio_select_editor('desc',$mc['editor'],$myts->htmlSpecialChars($catego->getVar('desc')),'100%','250px')));
    $form->addElement(new RMLabel(_MA_PORTFOLIO_DESC, $formmnote->render()));
